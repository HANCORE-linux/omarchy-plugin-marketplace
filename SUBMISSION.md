...
Use the [**Request plugin verification** issue form](https://github.com/HANCORE-linux/omarchy-plugin-marketplace/issues/new?template=verify-plugin.yml) instead of opening another submission. The request must identify the existing plugin ID, repository, and full `listingValidatedCommit` exactly. The automated workflow reruns the static baseline only for that recorded commit and publishes `Verified` only after a complete `passed` result with no findings or review capabilities. Other results remain `Unverified`; this is not a claim that the plugin is malicious. Read [Automated Plugin Verification](VERIFICATION.md) for the complete status definition, process, and limits.
