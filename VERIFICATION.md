...
If a `maintainer-verified` label was applied in error and later removed, the original review evidence remains preserved with an exact, event-bound revocation. The affected snapshot is projected as `Unverified`; a later review requires a fresh eligible bot report and a new label event after the revocation. A revocation never silently deletes historical evidence.
