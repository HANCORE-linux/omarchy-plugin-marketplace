...
Automated Security Baseline V1 statically scans the exact validated commit without executing plugin code. Preserve its deterministic outcomes (`passed`, `review-required`, and `needs-fixes`), evidence templates, exact-SHA binding, and required disclaimer. During the initial shadow period, both `review-required` and `needs-fixes` use `security-review-required`; do not apply the reserved `security-needs-fixes` label or make findings block approval until enforcement is explicitly enabled after real-submission measurement. Scan failures remain fail-closed. Treat executable binaries as review capabilities, and treat execution sourced only from the submitted repository as self-installation requiring review rather than as external mutable-code rejection.

The baseline is a listing-time check. Scheduled catalog refreshes still inspect upstream branch HEAD for compatibility and do not refresh the stored baseline. Do not describe that stored result as applying to later upstream commits.
