...
The Automated Security Baseline statically scans the exact validated commit without executing plugin code. The **Automated Security Baseline** section in `SECURITY.md` is the public policy for all contributors; keep implementation, reports, tests, and contributor documentation aligned with it. Preserve deterministic outcomes (`passed`, `review-required`, and `needs-fixes`), evidence templates, exact-SHA binding, and the required disclaimer. Do not use AI to determine outcomes, enforcement, labels, or approval. Selective enforcement still determines validation label disposition, but every new listing must now be published through `approved-and-verified`: a current `passed` result is automatic, a complete capability-only `review-required` result requires exact authorized maintainer attestation, and every finding or scan failure blocks initial publication until a new validated commit resolves it. Treat executable binaries as review capabilities, narrowly scoped root-owned helpers as review cases rather than automatic blockers, and execution sourced only from the submitted repository as self-installation requiring review rather than as external mutable-code rejection.
...
The registry stores automated baseline facts, including record schema, repository, affected plugin IDs, finding IDs, and capability IDs. Keep authorized maintainer verification separate as a canonical `maintainerVerificationReview` attestation bound to the exact bot-authored pre-label report, baseline repository, plugin IDs, commit, policy version, enforcement mode, prior and current scan times, outcome, empty finding set, accepted capability set, and exact label-event identity and request time. Initial submissions use `approved-and-verified`; existing listings use `maintainer-verified`. Both actions must reuse the same attestation validator and exact-evidence rules. Store the reviewer and review time for auditability, but never add a freely editable `verified` flag. Maintainer verification is eligible only for a complete current `review-required` result with no findings or scan error; `needs-fixes`, failed scans, stale evidence, and mismatches have no review bypass. The baseline is a listing-time check. Scheduled catalog refreshes still inspect upstream branch HEAD for compatibility and do not refresh the stored baseline. Do not describe that stored result as applying to later upstream commits. Derive `Verified` from either a current-version `passed` baseline with empty finding and capability sets or an exact eligible maintainer-review attestation; otherwise derive `Unverified`. Both paths describe only the exact listed commit, never trust or later upstream code. After either valid path succeeds, the responsible approval or verification workflow automatically tests and publishes the canonical registry/catalog update and deploys the tested Pages artifact.
