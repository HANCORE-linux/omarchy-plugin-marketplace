...
Automated Security Baseline V1 statically scans the exact validated commit without executing plugin code. `SECURITY_BASELINE.md` is the public policy for all contributors; keep implementation, reports, tests, and contributor documentation aligned with it. Preserve deterministic outcomes (`passed`, `review-required`, and `needs-fixes`), evidence templates, exact-SHA binding, and the required disclaimer. Do not use AI to determine outcomes, labels, or approval. During the initial shadow period, both `review-required` and `needs-fixes` use `security-review-required`; do not apply the reserved `security-needs-fixes` label or make findings block approval until enforcement is explicitly enabled after real-submission measurement. Scan failures remain fail-closed. Treat executable binaries as review capabilities, and treat execution sourced only from the submitted repository as self-installation requiring review rather than as external mutable-code rejection.

The registry stores automated baseline facts, including finding and capability IDs, but no maintainer identity, review timestamp, review flag, or other copied approval provenance. The baseline is a listing-time check. Scheduled catalog refreshes still inspect upstream branch HEAD for compatibility and do not refresh the stored baseline. Do not describe that stored result as applying to later upstream commits.
